...
export default function GHPPage() {
  ...
  const [schedules, setSchedules] = useState([]); const [saving, setSaving] = useState(false); const [submitted, setSubmitted] = useState(false); const [emailSent, setEmailSent] = useState(true); const [loading, setLoading] = useState(true);
  ...
  async function submit(event) { event.preventDefault(); if (!form.name.trim() || !isValidEmail(form.email) || !form.seminarDate) return showToast("Please enter your full name, a valid email address, and select a seminar schedule.", true); setSaving(true); try { const response = await fetch("/api/ghp/appointment", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) }); const data = await response.json(); if (!response.ok || !data.success) throw new Error(data.error); setEmailSent(data.emailSent !== false); setSubmitted(true); showToast(data.warning || "Your GHP seminar booking has been saved. A confirmation email has been sent.", Boolean(data.warning)); } catch (error) { showToast(error.message || "Unable to submit your booking.", true); await loadSchedules(); } finally { setSaving(false); } }
  ...
  return <>
    ...
    <main className={styles.page}><div className="container">
      ...
      <section className={styles.appointmentCard}><h2>Book a GHP seminar schedule</h2>{submitted ? <div className={styles.success}>Your booking has been received for <strong>{fmt(form.seminarDate)}</strong>. {emailSent ? "A confirmation email has been sent with your selected seminar schedule." : "Please contact NMIS RTOC III because the confirmation email could not be sent."} NMIS will email your examination result after it is manually recorded.</div> : <form className={styles.appointmentForm} onSubmit={submit}>
