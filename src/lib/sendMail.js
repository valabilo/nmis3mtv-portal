...
export async function sendGHPExamResult(record) {
  ...
  const passed = String(record.exam_result || "").trim().toUpperCase() === "PASSED";
    const subject = passed ? "GHP Examination Result: Passed" : "GHP Examination Result: Not yet passed";
    const result = passed ? "PASSED" : "NOT YET PASSED";
  const nextStep = passed
    ? "Your certificate has been generated by NMIS. Please coordinate with the office for collection."
