...
const DEFAULT_PURPOSE_PREFIX = "MTV Payment";
...
function plateNumber(application = {}) {
  return String(
    application.plate ||
      application.plateNo ||
...
function paymentPurpose(application = {}) {
  const plate = plateNumber(application);
  return plate ? `${DEFAULT_PURPOSE_PREFIX} for ${plate}` : DEFAULT_PURPOSE_PREFIX;
}

function applyAutoGeneratedWatermark(doc, label = "AUTO-GENERATED") {
  ...
  for (let pageIndex = pages.start; pageIndex < pages.start + pages.count; pageIndex += 1) {
    ...
    doc
      ...
      .text(label, 0, doc.page.height / 2 - 36, {
...
export async function generateOrderOfPaymentPdf(
  ...
  applyAutoGeneratedWatermark(doc);
...
export function buildOrderOfPaymentData(application = {}, onlinePayment = {}) {
  ...
  const purpose = paymentPurpose(application);
